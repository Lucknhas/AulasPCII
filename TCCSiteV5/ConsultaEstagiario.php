...
<?php
	...
	if(!isset($_SESSION["prontuario"]) || !isset($_SESSION["senha"])) {
		...
		$conn;
		$select = "SELECT * FROM tbestagiario ";
...
<html>
	<head>
		<meta charset="utf-8">
		<title>Consulta de Estagiário - SAP</title>
	...
	<body>
		...
		<div id="titulo-principal">
			<caption><h1>Lista dos Estagiários Cadastrados<br/>ETIM Centro Paula Souza</h1></caption>
		</div>
		...
		<table id="tabela-consulta-estagiario">
			...
			<tr>
				<td><?php echo $dado["idEstagiario"];?></td>
				<td><?php echo $dado["nome"];?></td>
				<td><?php echo $dado["rg"];?></td>
				<td><?php echo $dado["setor"];?></td>
				<td><?php echo date("d/m/Y", strtotime($dado["inicio"]));?></td>
				...
				<td>
					<a href="EditarEstagiario.php?codigo=<?php echo $dado["idEstagiario"]; ?>">Editar</a>
					<a href="DeletarEstagiario.php?codigo=<?php echo $dado["idEstagiario"]; ?>">Deletar</a>
				</td>
			</tr>
			<?php } ?>
		</table>
